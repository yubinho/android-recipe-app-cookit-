...
public class MenuLunchAdapter extends FirebaseRecyclerAdapter<Menu, MenuLunchAdapter.myviewholder> {
    String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();
    private Activity activity;
    public MenuLunchAdapter(@NonNull FirebaseRecyclerOptions<Menu> options, Activity activity){
        super(options);
            ...
            public void onClick(View v) {
                ...
                alertDialog.setPositiveButton("確定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        try{
                            FirebaseDatabase.getInstance().getReference("customMenu").child(userId).child("午餐")
    ...
    @NonNull
    @Override
    public MenuLunchAdapter.myviewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.menu_singlerow,parent,false);
        return new MenuLunchAdapter.myviewholder(view);
    }
